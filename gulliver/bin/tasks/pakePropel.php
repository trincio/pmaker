...
{
  switch ( $args [0]) {
    case 'mysql' : _call_phing($task, 'sql', true, 'propel.mysql.ini' );
                   break;
...
function run_propel_insert_sql($task, $args)
{
  switch ( $args [0]) {
    case 'mysql' : //_call_phing($task, 'insert-sql', true, 'propel.mysql.ini' );
                   ...
                   exec ( 'mysqldump --compatible=mysql40 --compact --default-character-set=utf8 --no-create-info  --complete-insert=false --extended-insert=true > data/mysql/insert.sql ' . $database );
                   break;
