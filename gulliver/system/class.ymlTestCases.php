 ...
 *     Output:
 *       Type: "TypeOfResult"          Tipo de dato que deberia tener el resultado.
 *       Value: "a:0{}"                Este nodo debe contener un valor serializado que sera comparado con el resultado de la prueba.
 *   -                                 Otro caso de prueba.
 *     ...
 * TestCasesGroup2:
 *   ...
...
class ymlTestCases
{
  ...
  function ymlTestCases( $testCaseFile, &$testDomain, &$testLime )
  {
    ...
    if (basename($testCaseFile)===$testCaseFile) $testCaseFile =  PATH_FIXTURES . $testCaseFile;
        ...
        else
        {
          if(isset($testCase['Output']))
          {
            if ( isset($testCase['Output']['Type']) ||
                 isset($testCase['Output']['Value']) ) $this->testLime->plan++;
          }
        }
      }
      else
  ...
  function run( &$testerObject, $fields=array(), $count=-1, $start=0 )
  {
    ...
    foreach($functions as $id=>$fn) $functions[$id]=strtolower($fn);
