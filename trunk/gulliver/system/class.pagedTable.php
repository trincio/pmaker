  ...
  function analizeSql()
  {
    ...
    if ($this->sqlOrderBy!='')
    {
      if ($n=preg_match_all('/\b([\w\.]+)\b(?:\s+(ASC|DESC))?,?/im',$this->sqlOrderBy,$matches,PREG_SET_ORDER))
      {
        for($r=0;$r < $n;$r++)
        {
          if (!isset($matches[$r][2])) $matches[$r][2]='';
          if ($matches[$r][2]=='') $matches[$r][2]='ASC';
        ...
        //Orden ascendente
        if ($n==1) $this->order=G::createUID('',$matches[0][1]) . '=' . $matches[0][2];
      }
    }
    ...
    //Prepare the fields
      ...
      foreach ($table->Columns as $r => $value)
      {
        ...
        if ($this->fields[$r]['Label']=='')
         ...
         {
           case 'image':case 'image-text': case 'jslink':
...
//    var_dump2($this);
  ...
  function renderField($row,$r,$result)
  {
      ...
      switch (true)
      {
        case ($this->style[$r]['data']!=''): $value = ((isset($result[$this->style[$r]['data']]))?$result[$this->style[$r]['data']]:'');
        break;
        default: $value = $this->fields[$r]['Label'];
      }
      switch ($this->fields[$r]['Type'])
      {
        case 'date':
          ...
          if (strlen ($value) <=10 && strlen ($value) > 4 ) {
            ...
            if (strpos($value,'-')===FALSE) $value = substr($value,0,4).'-'.substr($value,4,2).'-'.substr($value,6,2);
          }
      }
...
//    for($r=1;$r<=sizeof($this->fields);$r++)
  ...
  function renderTable( $block = '' )
  {
    ...
    // DBConnection
    $this->prepareQuery();
    //Query para obtener el numero de filas
    $this->query=$this->ses->execute($this->sql);
